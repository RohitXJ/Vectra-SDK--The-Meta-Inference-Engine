// CONFIGURATION: These values can be replaced during CI/CD or set manually
const CONFIG = {
    API_BASE_URL: window.API_BASE_URL || "", // Set via environment or script
    HF_TOKEN: window.HF_TOKEN || ""           // Required if Space is Private
};
...
// HELPER: Fetch with optional Authorization header
...
async function init() {
    ...
    try {
        const response = await authenticatedFetch(`${API_BASE}/session/new`, { method: 'POST' });
...
async function runLiveEval(file) {
    ...
    try {
        ...
        const res = await authenticatedFetch(`${API_BASE}/eval`, { method: 'POST', body: fd });
